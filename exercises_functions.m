...
if type == 3
    ...
    for i=1:N_episodes
        ...
        for j=2:N_steps
            %Apply to dynamics - Observe next state
            u(i,j-1) = (H(2,1)/H(2,2))*(final_state-x(i,j-1))+0.02*randn(1,1);
            K_vec = [ K_vec;(H(2,1)/H(2,2))];
            if x(i,j-1)<=2.5
                x(i,j) = a1*x(i,j-1)^p+b1*u(i,j-1);
            else
...
if type == 4
    ...
    for i=1:N_episodes
        ...
        for j=2:N_steps
            %Apply to dynamics - Observe next state
            u(i,j-1) = (H(2,1)/H(2,2))*(final_state-x(i,j-1))+0.5*randn(1,1);
            K_vec = [ K_vec;(H(2,1)/H(2,2))];
%             %-----
            scatter(j,K_vec(end))
            hold on
...
%             -----
            ...
            matrix = [error^2 u(i,j-1)*error u(i,j-1)*error u(i,j-1)^2];

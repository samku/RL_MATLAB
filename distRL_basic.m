...
for i=1:N_MC
    ...
    %the values of the H matrices - Minimize the quadratic cost in H
    u1_o = [];
    u2_o = [];
    for j=1:N_sim-1
        disp('-------------------------')
        ...
        u2_opt = sdpvar(1,1);
        f = sdpvar(1,1);
        f = [x1_vec(j) u1_opt x2_vec(j) u2_opt]*[H_1 H_12; H_21 H_2]*[x1_vec(j);u1_opt;x2_vec(j);u2_opt];
        optimize([],f,sdpsettings('verbose',0));
        u1_o = [u1_o value(u1_opt)];
        u2_o = [u2_o value(u2_opt)];
        disp('Yalmip minim ----')
        [value(u1_opt);value(u2_opt)]
        disp('Gain matrix--------')
        gains*[x1_vec(j);x2_vec(j)]
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
